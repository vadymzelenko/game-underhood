...
export class WorldScene extends Phaser.Scene {
    ...
    create() {
        ...
        this.grid = new WorldGrid(this, TUNING.world.gridCell);
        ...
        this._obsCell = TUNING.world.obstacleCell;
        this._obsDirty = true;
    ...
    _findSafeSpawn(startX, startY) {
        ...
        const step = TUNING.world.spawnStep;
        for (let r = step; r < TUNING.world.spawnMaxR; r += step) {
    ...
    _findFreeSpotNear(x, y, maxR = TUNING.world.freeSpotMaxR) {
    ...
    _doTeleport(target) {
        ...
        // Перегенерируем чанки под новой точкой, затем обновим индекс препятствий.
        this.chunkManager.update(target.x, target.y, null, null);
        this._obsDirty = true;

        if (this.isBlockedAt(target.x, target.y, true)) {
            const free = this._findFreeSpotNear(target.x, target.y);
